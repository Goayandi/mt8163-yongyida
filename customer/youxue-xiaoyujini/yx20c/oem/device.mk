...
ifeq ($(strip $(MTK_UIPQ_SUPPORT)), yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.mtk_uipq_support=0
else
  PRODUCT_PROPERTY_OVERRIDES += ro.mtk_uipq_support=1
endif
...
ifeq ($(strip $(MTK_IMS_SUPPORT)), yes)
  ...
  PRODUCT_PROPERTY_OVERRIDES += ro.mtk.volte.enable=1
endif
...
ifneq ($(MTK_AUDIO_TUNING_TOOL_VERSION),)
  ifneq ($(strip $(MTK_AUDIO_TUNING_TOOL_VERSION)),V1)
    ...
    AUDIO_PARAM_OPTIONS_LIST += 5_POLE_HS_SUPPORT=$(MTK_HEADSET_ACTIVE_NOISE_CANCELLATION_SUPPORT)
...
ifeq ($(strip $(YYD_AUTO_UPDATE_SUPPORT)), yes)
    PRODUCT_PACKAGES += YYDRobotAppUpdate  #应用升级 
endif

ifeq ($(strip $(YYD_BREATHLED_SUPPORT)), yes)
    PRODUCT_PACKAGES += YYDRobotBreathledService   #呼吸灯服务
    PRODUCT_PACKAGES += YYDRobotBreathLed   #呼吸灯
endif

ifeq ($(strip $(YYD_PROJECTION_SUPPORT)), yes)
    PRODUCT_PACKAGES += YYDRobotLightPenTest    # 投影光电笔校准
    PRODUCT_PACKAGES += YYDRobotProjectionTouch # 投影光电笔使用
endif

ifeq ($(strip $(YYD_FPGA_APK_SUPPROT)), yes)
...
ifeq ($(strip $(YYD_AR_SUPPORT)), yes)
    PRODUCT_PACKAGES += YYDRobotXYAR     # 小勇AR
endif

PRODUCT_PACKAGES += YYDRobotIDHelper     # 获取机器人ID。删除时请谨慎！会造成ID读取失败
...
PRODUCT_PACKAGES += YYDRobotLockScreen     
PRODUCT_PACKAGES += YYDRobotPhotos
PRODUCT_PACKAGES += YYDRobotQrCode
PRODUCT_PACKAGES += YYDRobotVideo
PRODUCT_PACKAGES += YYDRobotVideoPlayer
#PRODUCT_PACKAGES += YYDRobotVoiceBattery
PRODUCT_PACKAGES += YYDRobotVoiceCamera
PRODUCT_PACKAGES += YYDRobotVoiceNotification
PRODUCT_PACKAGES += YYDRobotGameCenter
PRODUCT_PACKAGES += YYDMoliVideo              
PRODUCT_PACKAGES += YYDRobotXMMusicSearch
